...
parser.add_argument("--output_usd_dir", type=str, default="datasets/usd_recordings", help="Root directory where usd files to be exported.")
...
def main():
    ...
    # Set up for USD export if needed
    usd_export_enabled = False
    if args_cli.output_usd_dir is not None:
        if args_cli.num_envs > 1:
            raise ValueError("USD export is only supported with a single environment.")
        # Create output directory for USD files if not existed
        if not os.path.exists(args_cli.output_usd_dir):
            os.makedirs(args_cli.output_usd_dir)
        env_cfg.sim.use_fabric = False
        usd_export_enabled = True
            ...
            while has_next_action:
                ...
                for env_id in range(num_envs):
                    ...
                    if env_next_action is None:
                        ...
                        # Stop/conclude USD recorder if it is in recording
                        if is_recording_usd:
                            print("\tFinializing USD recording for this episode...")
                            current_usd_dir_path = env.unwrapped._window.animation_log_dir
                            env.unwrapped._window._toggle_recording_animation_fn(False)
                            is_recording_usd = False
                            print(f"\tUSD files exported (exported episodes count = {usd_export_count})")
                        ...
                        if next_episode_index is not None:
                            ...
                            if usd_export_enabled:
                                # Start USD recording USD for the new episode
                                usd_export_count += 1
                                env.unwrapped._window.animation_log_dir = os.path.join(
                                    args_cli.output_usd_dir, str(usd_export_count)
                                )
                                env.unwrapped._window._toggle_recording_animation_fn(True)
                                is_recording_usd = True
                        else:
                            continue
                    else:
                        has_next_action = True
    ...
    # Conclude the last usd recording if it is enabled
    if is_recording_usd:
        env.unwrapped._window._toggle_recording_animation_fn(False)
        is_recording_usd = False
        print(f"Finished exporting {usd_export_count} USD file sets to {args_cli.output_usd_dir}")
